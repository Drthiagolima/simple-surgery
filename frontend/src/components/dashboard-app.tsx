...
const scannerEventOptions = [
  ...
  { key: "rpa_exit", label: "Saida RPA" },
  { key: "room_entry", label: "Entrada Sala" },
  { key: "room_exit", label: "Saida Sala" },
  { key: "cc_exit", label: "Saida CC" },
];
...
const scannerFeedbackDetails = {
  ...
  attention: {
    title: "Atencao no fluxo",
    subtitle: "Evento fora da sequencia padrao. Revisar antes de confirmar.",
    chipClass: "border-amber-300/35 bg-amber-300/15 text-amber-100",
  },
  error: {
    title: "Erro de leitura",
    subtitle: "QR nao reconhecido. Reposicione a camera e tente novamente.",
...
export function DashboardApp() {
  ...
  const roomTimelineSteps = [
    ...
    { label: "Inicio cirurgia", occurredAt: activeRoomSurgery?.room_entry_at },
    { label: "Saida Sala", occurredAt: activeRoomSurgery?.room_exit_at },
    { label: "Saida CC", occurredAt: activeRoomSurgery?.cc_exit_at },
  ];
  ...
  const sectionSubtitle = activeSection === "scanner"
    ? "Registro rapido de eventos operacionais com feedback visual imediato para o fluxo da cirurgia."
    : activeSection === "rooms"
      ? "Painel detalhado da sala com status atual, paciente e cronologia operacional."
    : activeSection === "timeline"
      ? "Rastreamento completo do fluxo cirurgico com sequencia de eventos e tempos operacionais."
      : "Centro Cirurgico - visao operacional do dia com salas, fila e eventos em tempo real.";

  const sectionBadge = activeSection === "scanner"
    ? "Ambiente de captura"
    : activeSection === "timeline"
      ? "Rastreabilidade cirurgica"
      : activeSection === "rooms"
        ? "Operacao por sala"
        : "Visao geral";
  ...
  async function hydrateAll(accessToken: string, fallbackUser?: AuthUser) {
    ...
    if (!user) {
      handleLogout();
      setLoadError("Sua sessao expirou. Entre novamente para acessar o painel.");
      setIsBootstrapping(false);
      return;
    }
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setLoadError(error instanceof Error ? error.message : "Nao foi possivel autenticar.");
  ...
  async function handleCreateSurgery(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setActionError(error instanceof Error ? error.message : "Nao foi possivel criar cirurgia.");
    } finally {
  ...
  async function handleCreateEvent(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setActionError(error instanceof Error ? error.message : "Nao foi possivel registrar evento.");
    } finally {
          ...
          <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
            <section className="ss-panel rounded-[2rem] p-8">
              ...
              <p className="mt-4 max-w-xl text-base text-muted-foreground">
                Painel autenticado para operacao do centro cirurgico com controle por eventos, leitura de salas e fila
                cirurgica em tempo real.
              </p>
              <div className="mt-8 grid gap-4 sm:grid-cols-3">
                <FeatureTile icon={<Activity className="h-4 w-4" />} title="Eventos" text="Trilha RPA e sala" />
                <FeatureTile icon={<DoorClosed className="h-4 w-4" />} title="Salas" text="Ocupacao e ociosidade" />
      ...
      <div className="mx-auto grid w-full max-w-[1680px] gap-4 px-3 py-3 lg:grid-cols-[244px_1fr] lg:px-5">
        <aside className="ss-panel hidden p-4 lg:flex lg:flex-col">
          ...
          <div className="mt-auto rounded-2xl border border-border/70 bg-background/70 p-4">
            <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Sessao ativa</p>
        ...
        {activeSection === "dashboard" ? (
          <>
            ...
            {!payload.apiAvailable && !isBootstrapping ? (
              <section className="mb-8 rounded-3xl border border-amber-300/40 bg-amber-300/10 p-5 text-amber-100 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold">API autenticada ainda nao respondeu com dados.</p>
                    <p className="mt-1 text-sm text-amber-100/80">
                      A sessao JWT foi aceita, mas o dashboard ainda nao recebeu overview, salas ou cirurgias da API.
                    </p>
                  </div>
                </div>
              </section>
            ...
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <MetricCard icon={<Clock3 className="h-4 w-4" />} label="Tempo medio RPA" value={formatMinutes(overview.avg_rpa_minutes)} hint="Tempo medio entre entrada e saida da RPA" />
              <MetricCard icon={<DoorClosed className="h-4 w-4" />} label="Salas monitoradas" value={String(payload.rooms.length)} hint="Ocupacao e sala parada por ambiente" />
              ...
              <div className="ss-panel p-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">Visao de disponibilidade, ocupacao do dia e janela de ociosidade por sala.</p>
                  </div>
                ...
                <div className="mt-5 grid gap-4">
                  {payload.rooms.length > 0 ? (
                      ...
                      <article key={room.room_id} className="rounded-2xl border border-border bg-background/65 p-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <div className="flex gap-3 text-sm">
                            <RoomStat label="Ocupacao" value={formatMinutes(room.occupancy_minutes_today)} />
                    ...
                    <EmptyState title="Nenhuma sala encontrada" description="As salas aparecerao aqui quando o backend receber dados operacionais." />
                  )}
                </div>
              </div>

              <div className="ss-panel p-6">
                <h2 className="text-xl font-bold">Indicadores de eficiencia</h2>
                <p className="mt-2 text-sm text-muted-foreground">A tabela central operational_events alimenta status, tempos e auditoria do centro cirurgico.</p>
                <div className="mt-5 space-y-4">
                  <KpiRow label="Concluidas" value={String(overview.surgeries_completed)} />
                  <KpiRow label="Tempo medio de sala" value={formatMinutes(overview.avg_room_minutes)} />
  ...
  "event_type": "room_entry"
            ...
            <section className="mt-8 grid gap-6 lg:grid-cols-2">
              <section className="ss-panel p-6">
                ...
                <form className="space-y-4" onSubmit={handleCreateSurgery}>
                  ...
                  <div>
                    <FieldLabel htmlFor="surgeon_name">Cirurgiao</FieldLabel>
                  ...
                  <div>
                    <FieldLabel htmlFor="scheduled_start">Inicio previsto</FieldLabel>
              ...
              <section className="ss-panel p-6">
                ...
                <form className="space-y-4" onSubmit={handleCreateEvent}>
                  ...
                  <div>
                    <FieldLabel htmlFor="occurred_at">Horario do evento (opcional)</FieldLabel>
            ...
            <section className="mt-8 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
              <div className="ss-panel p-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold">Fila cirurgica</h2>
                ...
                <div className="mt-5 space-y-3">
                  {payload.surgeries.length > 0 ? (
                    payload.surgeries.slice(0, 6).map((surgery) => (
                      <article key={surgery.id} className="rounded-2xl border border-border bg-background/65 p-4">
                        ...
                        <div className="mt-3 grid gap-2 text-sm text-muted-foreground sm:grid-cols-3">
                          <span>Inicio previsto: {formatDateTime(surgery.scheduled_start)}</span>
                    ...
                    <EmptyState title="Sem cirurgias carregadas" description="Assim que o backend tiver registros, a fila operacional aparecera aqui." />
                  )}
                </div>
              </div>
              ...
              <div className="ss-panel p-6">
                ...
                <div className="mt-4 rounded-2xl border border-border bg-background/65 p-4">
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div>
                      <FieldLabel htmlFor="analytics_period_days">Periodo</FieldLabel>
                      <select
                      ...
                      <div key={`${entry.item.event_type}-${entry.item.occurred_at}-${index}`} className="rounded-xl border border-border bg-background/65 px-3 py-2">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className={entry.isValid ? "rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-800" : "rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-amber-800"}>
                            {entry.isValid ? "sequencia ok" : "fora de ordem"}
                          </span>
                        </div>
          ...
          <section className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
                  ...
                  <div className="mx-auto mt-10 max-w-md rounded-2xl border border-border bg-white/90 px-4 py-3 text-neutral-900">
                    <div className="grid grid-cols-[1fr_auto] items-center gap-3">
                      <div>
                        ...
                        <p className="text-xs text-neutral-600">Prontuario: 456789 | Sala: 01</p>
                      </div>
                      <QrCode className="h-16 w-16" />
                    </div>
                  </div>

                  <p className="mt-8 text-center text-sm text-muted-foreground">Centralize o QR Code na area indicada</p>
                </div>
                ...
                <div className={`mt-4 rounded-2xl border p-4 ${scannerFeedbackDetails[scannerFeedback].chipClass}`}>
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-lime-100/80">Cirurgiao: Dr. Thiago Lima | Sala 01</p>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              <div className="ss-panel p-5 ss-stagger" style={{ animationDelay: "80ms" }}>
                ...
                <div className="mt-3 space-y-3">
                  ...
                  <button className="w-full text-left" onClick={() => setScannerFeedback("attention")} type="button">
                    <ScannerStatusCard active={scannerFeedback === "attention"} icon={<AlertTriangle className="h-5 w-5" />} title="Atencao" text="Sequencia inesperada no fluxo" tone="warning" />
                  </button>
                  <button className="w-full text-left" onClick={() => setScannerFeedback("error")} type="button">
                    <ScannerStatusCard active={scannerFeedback === "error"} icon={<XCircle className="h-5 w-5" />} title="Erro" text="QR Code nao reconhecido" tone="error" />
                  </button>
                </div>
              </div>

              <div className="ss-panel p-5 ss-stagger" style={{ animationDelay: "120ms" }}>
                <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Proximo evento sugerido</p>
                <div className="mt-3 rounded-2xl border border-border bg-background/70 p-4">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <p className="text-xl font-semibold">Saida RPA</p>
              ...
              <div className="ss-panel p-5 ss-stagger" style={{ animationDelay: "160ms" }}>
                <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Fluxo rapido</p>
                ...
                <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-semibold">
                      {timelinePatient?.full_name ?? "Paciente nao identificado"}
                    </h2>
                    <p className="mt-2 text-sm text-muted-foreground">
                      Prontuario: {timelinePatient?.medical_record_number ?? "--"}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Cirurgiao: {timelineSurgery?.surgeon_name ?? "--"}
                    </p>
                  </div>
                  <div className="rounded-2xl border border-border bg-background/65 p-4">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">Inicio previsto: {formatDateTime(timelineSurgery?.scheduled_start ?? null)}</p>
            ...
            <aside className="space-y-6">
              ...
              <section className="ss-panel p-5">
                ...
                <div className="mt-4 grid gap-2">
                  <Button variant="secondary"><FileText className="h-4 w-4" /> Gerar relatorio do caso</Button>
        ...
        {activeSection === "rooms" ? (
          <section className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
            <div className="space-y-6">
              <section className="ss-panel p-5 sm:p-6 ss-stagger">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                  <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Visao da sala</p>
                  <div className="min-w-[260px]">
                ...
                <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                  <div className="rounded-2xl border border-border bg-background/65 p-4">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">Prontuario: {roomPatient?.medical_record_number ?? "--"}</p>
                    <p className="text-sm text-muted-foreground">Procedimento: {activeRoomSurgery?.procedure_name ?? "--"}</p>
                    <p className="text-sm text-muted-foreground">Cirurgiao: {activeRoomSurgery?.surgeon_name ?? "--"}</p>
                  </div>
                ...
                <div className="mt-4 space-y-3">
                  {roomSurgeries.length ? roomSurgeries.slice(0, 5).map((surgery) => (
                    <article key={surgery.id} className="rounded-2xl border border-border bg-background/65 p-4">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">{surgery.surgeon_name}</p>
                      <p className="mt-1 text-xs text-muted-foreground">Inicio previsto: {formatDateTime(surgery.scheduled_start)}</p>
                    </article>
                  )) : (
                    <EmptyState title="Sem registros operacionais" description="Quando houver cirurgias vinculadas a esta sala, elas aparecerao aqui." />
                  )}
                </div>
              </section>
            </div>
            ...
            <aside className="space-y-6">
              <section className="ss-panel p-5">
                <h3 className="text-lg font-semibold">Cronometros e metricas</h3>
                <div className="mt-4 space-y-3">
              ...
              <section className="ss-panel p-5">
                <h3 className="text-lg font-semibold">Proximo paciente</h3>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Inicio previsto: {formatDateTime(nextRoomSurgery?.scheduled_start ?? null)}
                </p>
              </section>
              ...
              <section className="ss-panel p-5">
                ...
                <div className="mt-4 space-y-2 text-sm text-muted-foreground">
                  ...
                  <div className="rounded-xl border border-border bg-background/65 p-3">
                    Ultima atualizacao: {dashboardDate}
                  </div>
                </div>
              </section>
            </aside>
        ...
        {activeSection !== "dashboard" && activeSection !== "scanner" && activeSection !== "timeline" && activeSection !== "rooms" ? (
          <section className="ss-panel p-8">
            <h2 className="text-2xl font-semibold">Ambiente em preparacao</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              Esta area sera entregue nas proximas etapas com o mesmo padrao visual do mockup.
            </p>
          </section>
        ) : null}
        </main>
...
function translateEventType(eventType: string): string {
  const labels: Record<string, string> = {
    ...
    rpa_exit: "Saida da RPA",
    room_entry: "Entrada na sala",
    room_exit: "Saida da sala",
    cc_exit: "Saida do centro cirurgico",
  };
